...
static OUTPUTMANAGER OutMgr;
static DISPLAYMANAGER DispMgr;
static THREADMANAGER ThreadMgr;
static DUPLICATIONMANAGER DuplMgr;
static HWND WindowHandle;
static THREAD_DATA* TData;
static BOOL ReadyRender = FALSE;
...
// Displays a message
//
static void DisplayMsg(_In_ LPCWSTR Str, _In_ LPCWSTR Title, HRESULT hr)
{
    if (SUCCEEDED(hr))
    {
...
static void MessageThread() 
{
    ...
    INT SingleOutput = 0; // all or nths
    // Synchronization
     ...
     while (WM_QUIT != msg.message)
    {
        ...
        else if (FirstTime || WaitForSingleObjectEx(ExpectedErrorEvent, 0, FALSE) == WAIT_OBJECT_0)
        {
            ...
            if (Ret == DUPL_RETURN_SUCCESS)
            {
                ...
                if (SharedHandle)
                {
                    ...
                    // threadData.Output = SingleOutput;
                    // threadData.TexSharedHandle = SharedHandle;
                    // threadData.OffsetX = DeskBounds.left;
                    // threadData.OffsetY = DeskBounds.top;
                    // threadData.PtrInfo = &ptrInfo;

                    // DispMgr.InitD3D(&threadData);
        ...
        else
        {
            ...
            if (!Occluded)
            {
                Ret = OutMgr.UpdateApplicationWindow(nullptr, &Occluded);
            }
        }
...
DWORD WINAPI DDProc(_In_ void* Param)
{
    ...
    TData = reinterpret_cast<THREAD_DATA*>(Param);
    // New display manager
...
LRESULT Render::Init()
{
    message_thread.reset(new std::thread(MessageThread));
    return 0;
}

int Render::RenderTexture(ID3D11Texture2D *tex)
{
    if (!ReadyRender) return 0;
    ...
    // Process new frame
    Ret = DispMgr.ProcessFrameDirectly(&CurrentData, SharedSurf, TData->OffsetX, TData->OffsetY, &DesktopDesc);
    if (Ret != DUPL_RETURN_SUCCESS)
    {
...
Exit:
    ...
    return Ret == DUPL_RETURN_SUCCESS ? 0 : -1;
}
