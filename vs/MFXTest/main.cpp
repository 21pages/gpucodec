...
extern "C" {
    ...
    void* mfx_new_encoder(void* hdl, int64_t luid, API api,
        DataFormat dataFormat, int32_t width,
        int32_t height, int32_t kbs, int32_t framerate,
        int32_t gop, int32_t q_min, int32_t q_max);
    int mfx_encode(void* e, void* tex, EncodeCallback callback,
        void* obj);
    int mfx_destroy_encoder(void* e);

    void* mfx_new_decoder(int64_t luid, API api, DataFormat dataFormat,
    ...
    void* CreateDXGIRender(int64_t luid);
...
int main() {
    ...
    void* decoder =
        mfx_new_decoder(luid, API_DX11, dataFormat, SURFACE_FORMAT_BGRA);
    if (!decoder) {
