...
amf_int32               widthIn = 2880;
amf_int32               heightIn = 1800;
...
amf_wstring workAlgorithm = L"OneInOne";
amf_wstring fileNameOut = L"EncoderLatency.h264";
amf_wstring fileNameIn = L"D:\\tmp\\hw.yuv";
...
AMF_RESULT SetEncoderDefaults(ParametersStorage* pParams, amf::AMFComponent* encoder, const  amf_wstring& codecStr)
{
    ...
    else if (codecStr == amf_wstring(AMFVideoEncoder_HEVC))
    {
        ...
        res = encoder->SetProperty(AMF_VIDEO_ENCODER_HEVC_USAGE, AMF_VIDEO_ENCODER_HEVC_USAGE_ULTRA_LOW_LATENCY);
...
void PollingThread::Run()
{
    ...
    while (true)
    {
        ...
        if (data != NULL)
        {
            ...
            printf("diff:%.4f\n", (double)(poll_time - start_time) / MILLISEC_TIME);
...
extern "C" int amf_encode()
{
    ...
    else
    {
        ...
        while (submitted < frameCount)
        {
            ...
            //printf("surface op:%.4f ms\n", (double)(amf_high_precision_clock() - begin_frame) / MILLISEC_TIME);
        ...
        amf_pts end_time = amf_high_precision_clock();
        printf("submitCount:%d, dataCounter:%d\n",submitted, dataCounter);
        printTime(end_time - begin_time, latency_time, first_frame, min_latency, max_latency);
    }
